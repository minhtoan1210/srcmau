...
import { Button, Col, Collapse, Form, FormInstance, Input, InputNumber, Row, Space } from 'antd'
...
export const renderCriteriaItems = (
  criteria: AssignmentItemType,
  criteriaNames: number,
  functionRemove?: () => void
): JSX.Element => {
  return (
    <Row gutter={[16, 16]} style={{ width: '100%' }}>
      ...
      <Col style={{ display: 'flex', alignItems: 'center' }} span={functionRemove ? 4 : 6}>
        Positive:
        <CheckCircleTwoTone
          style={{ paddingLeft: 8 }}
      ...
      {functionRemove && (
        <Col style={{ display: 'flex', alignItems: 'center' }} span={2}>
          <Button onClick={functionRemove} icon={<DeleteOutlined />} danger />
        </Col>
      )}
    </Row>
  )
...
export default function AssignmentDetailFrom({ name, form }: Props): JSX.Element {
  useEffect(() => {
