  ...
  },
  {
    ...
    render: (value, record) => {
      return (
        <CheckCircleTwoTone
          onClick={() =>
            updateUserMutation.mutate({ id: record.uuid, dataToSubmit: { is_published: !value } })
          }
...
export default function MyFilesListPage(): JSX.Element {
  ...
  const { data, isLoading, refetch } = useQuery(['getMyFiles', paramQueryModel], () =>
    getUserFiles({ ...paramQueryModel, id: uuid })
  )

  const deleteUserFileMutation = useMutation(deleteUserFile, {
    onSuccess: () => {
  ...
  const onDeleteHandler = (id?: string): void => {
    if (id) {
      deleteUserFileMutation.mutate({ id: uuid, file_uuid: id })
    }
  }
  ...
  return (
    <Card
      ...
      <Select
        style={{ minWidth: 250, paddingBottom: 10, paddingTop: 0 }}
