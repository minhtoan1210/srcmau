...
import { Button, Card, Input, Select, Space, UploadProps } from 'antd'
...
export default function MyFileAddPage(): JSX.Element {
  ...
  const mutationUploadFile = useMutation(uploadUserFiles, {
    ...
    onSuccess: () => {
      ...
      navigate('/management/files-management/my-files')
    },
  })
  ...
  const onUploadFilesHandler = (file: RcFile): void => {
    ...
    if (uuid) {
      return mutationUploadFile.mutate({ formData, id: uuid })
    }
        ...
        <>
          <Space>
            <Tooltip title='Go Back'>
              <Button
                style={{ color: 'black' }}
                onClick={() => navigate('/management/files-management/my-files')}
                type='link'
                icon={<ArrowLeftOutlined />}
              />
            </Tooltip>
          </Space>
          {uuid ? <>&nbsp; User Files</> : 'User Files'}
        </>
      }
      actions={[
        <Button
