...
import { QueryParamsType, RoleType } from 'types'

import { EditFilled, PlusOutlined } from '@ant-design/icons'

import ConfirmDelete from 'components/ConfirmDeleteButton'

import { deleteRoles, getRoles } from 'services/systemServices'

const getColumns = (
  onDeleteHandler: (id?: string) => void,
  isDeleting: boolean
): TableColumnsType<RoleType> => [
  {
    title: 'Name',
    dataIndex: 'name',
    sorter: true,
  ...
  },
  {
    ...
    render: (_value, record) => {
      const confirmDelete = 'Are you sure you want to delete this Role'
      return (
        <>
...
export default function RoleListPage(): JSX.Element {
  ...
  const { data, isLoading, refetch } = useQuery(['getRoles', paramQueryModel], () =>
    getRoles(paramQueryModel)
  )
    ...
    <Card
      ...
      extra={
        <Space align='center'>
          ...
          />
          <Link to='add'>
            <Button icon={<PlusOutlined />}>Add Role</Button>
          </Link>
        </Space>
      }>
      <Table
        ...
        loading={isLoading}
        rowKey='id'
        columns={getColumns(onDeleteHandler, deleteRoleMutation.isLoading)}
